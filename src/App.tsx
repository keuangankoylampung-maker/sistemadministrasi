...
export default function App() {
  ...
  // Google Apps Script Auto-Sync Handlers
  const syncBatchedPayments = async (payments: LogPembayaran[]) => {
    const gasUrl = localStorage.getItem('SA_SPP_GAS_URL');
    if (!gasUrl) return;

    setSyncStatus({ 
      type: 'syncing', 
      message: `Mengunggah ${payments.length} transaksi ke Google Sheets...` 
    });
    ...
    for (let i = 0; i < payments.length; i++) {
      ...
      try {
        ...
        if (data && data.success) {
          ...
          throw new Error(data?.message || 'Gagal memproses baris log.');
        }
      } catch (err: any) {
        console.error('Batch sync failure at index', i, err);
        failedMsg = err.message || 'Koneksi bermasalah';
      }
    }
    ...
    if (successCount === payments.length) {
      setSyncStatus({
        type: 'success',
        message: `Sukses mengunggah seluruh (${successCount}) transaksi ke Google Sheets!`
      });
      setTimeout(() => setSyncStatus({ type: 'idle', message: '' }), 4000);
    } else {
      setSyncStatus({
        type: 'error',
        message: `Sinkronisasi parsial: ${successCount} berhasil, ${payments.length - successCount} gagal. (${failedMsg})`
      });
    }
  };

  const syncVoidKuitansi = async (noKuitansi: string) => {
    const gasUrl = localStorage.getItem('SA_SPP_GAS_URL');
    if (!gasUrl) return;

    setSyncStatus({ 
      type: 'syncing', 
      message: `Membatalkan seluruh transaksi Kuitansi ${noKuitansi} di Google Sheets...` 
    });
    ...
    try {
      ...
      if (data && data.success) {
        setSyncStatus({
          type: 'success',
          message: `Berhasil membatalkan Kuitansi ${noKuitansi} di Google Sheets!`
        });
        setTimeout(() => setSyncStatus({ type: 'idle', message: '' }), 4500);
      } else {
        throw new Error(data?.message || 'Aplikasi Google Apps Script menolak pembatalan.');
      }
    } catch (err: any) {
      console.error('GAS Void Sync Error:', err);
      setSyncStatus({
        type: 'error',
        message: `Gagal membatalkan kuitansi di Google Sheets: ${err.message || 'Koneksi bermasalah'}`
      });
    }
  };

  const syncGenerateBilling = async (bulanTahunValue: string) => {
    const gasUrl = localStorage.getItem('SA_SPP_GAS_URL');
    if (!gasUrl) return;

    setSyncStatus({
      type: 'syncing',
      message: `Mengirim perintah pembuatan tagihan ${bulanTahunValue} ke Google Sheets...`
    });
    ...
    try {
      ...
      if (data && data.success) {
        setSyncStatus({
          type: 'success',
          message: `Berhasil menggenerasi tagihan ${bulanTahunValue} di Google Sheets secara serentak!`
        });
        setTimeout(() => setSyncStatus({ type: 'idle', message: '' }), 4500);
      } else {
        throw new Error(data?.message || 'Aplikasi Google Apps Script menolak pembuatan tagihan.');
      }
    } catch (err: any) {
      console.error('GAS Billing Generation Sync Error:', err);
      setSyncStatus({
        type: 'error',
        message: `Gagal sinkronasi pembuatan tagihan di Google Sheets: ${err.message || 'Koneksi bermasalah'}`
      });
    }
  };
  ...
  const handleTriggerBillingGeneration = (bulanTahunValue: string, onConsoleLog: (msg: string) => void) => {
    ...
    if (newTagihanEntries.length > 0) {
      ...
      // Trigger background sync to Google Sheets Web App
      syncGenerateBilling(bulanTahunValue);
    }
  };
  ...
  const handleCommitPembayaran = (pembayaran: {
    ...
    // Trigger auto-upload upload to Google Sheets
    syncBatchedPayments(newLogs);
  ...
  const handleCancelPembayaran = (noKuitansi: string) => {
    ...
    // Trigger cancellation sync to Google Sheets
    syncVoidKuitansi(noKuitansi);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-slate-100 font-sans text-slate-800 overflow-hidden">
          ...
          <div className="ml-auto flex items-center gap-4">
            <div className="text-right hidden md:block">
              ...
              {syncStatus.type === 'syncing' ? (
                <span className="text-xs text-blue-600 font-bold flex items-center gap-1 justify-end mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-500 inline-block animate-ping"></span>
                  SINKRONISASI...
                </span>
              ) : syncStatus.type === 'success' ? (
                <span className="text-xs text-emerald-600 font-bold flex items-center gap-1 justify-end mt-0.5" title={syncStatus.message}>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block"></span>
                  BERHASIL UNGGAH!
                </span>
              ) : syncStatus.type === 'error' ? (
                <span className="text-xs text-rose-500 font-extrabold flex items-center gap-1 justify-end mt-0.5" title={syncStatus.message}>
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500 inline-block animate-pulse"></span>
                  GAGAL SYNC ⚠️
                </span>
              ) : localStorage.getItem('SA_SPP_GAS_URL') ? (
                <span className="text-xs text-emerald-500 font-bold flex items-center gap-1 justify-end mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse"></span>
                  TERHUBUNG (GAS)
                </span>
              ) : (
                <span className="text-xs text-slate-400 font-bold flex items-center gap-1 justify-end mt-0.5">
      ...
      {syncStatus.type !== 'idle' && (
        <div className={`fixed bottom-6 right-6 z-50 p-4 rounded-lg shadow-xl border flex items-center gap-3 transition-all duration-300 transform translate-y-0 max-w-sm ${
          syncStatus.type === 'syncing' ? 'bg-blue-50 border-blue-200 text-blue-900' :
          syncStatus.type === 'success' ? 'bg-emerald-50 border-emerald-250 text-emerald-950' :
          ...
          <div className="flex-1 text-left">
            <p className="text-[9px] font-extrabold uppercase tracking-wider text-slate-400 select-none">Sinkronisasi Google Sheets</p>
            <p className="text-xs font-bold leading-normal mt-0.5">{syncStatus.message}</p>
          </div>
          {syncStatus.type === 'error' && (
            <button 
