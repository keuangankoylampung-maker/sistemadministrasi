...
export default function ScriptHubTab() {
  ...
  const getLocalLogsCount = () => {
    try {
      const stored = localStorage.getItem('SA_SPP_LOGS');
      if (stored) {
  ...
  const handleBulkUpload = async () => {
    const storedLogsStr = localStorage.getItem('SA_SPP_LOGS');
    if (!storedLogsStr) {
      setBulkResult({
        success: false,
        message: 'Tidak ada data transaksi lokal yang disimpan untuk disinkronkan.'
      });
      return;
    }
    ...
    let localLogs = [];
    try {
      localLogs = JSON.parse(storedLogsStr);
      ...
      setBulkResult({
        success: false,
        message: 'Gagal membaca data transaksi lokal.'
      });
      return;
    }
    ...
    setBulkLoading(true);
    setBulkResult(null);
    setBulkProgress({ current: 0, total: localLogs.length });

    // Reverse logs to process earliest first if listed chronologically
    const logsToSync = [...localLogs].reverse(); 
    ...
    for (let i = 0; i < logsToSync.length; i++) {
      ...
      setBulkProgress({ current: i + 1, total: logsToSync.length });
      ...
      try {
        const response = await fetch(gasUrl, {
          ...
          body: JSON.stringify({
            action: 'prosesBayar',
            nisn: log.nisn,
            idTagihan: log.idTagihan,
            jumlahBayar: log.jumlahBayar,
            metodeBayar: log.metodeBayar,
            penerima: log.penerima,
            keterangan: log.keterangan || '-',
            tanggalManual: log.tanggal
          })
        });
        ...
        if (data && data.success) {
          ...
          throw new Error(data?.message || 'Apps Script gagal menyimpan baris.');
        }

      } catch (err: any) {
        console.error('Bulk sync err at index', i, err);
        failError = err.message || 'Masalah koneksi jaring atau format URL tidak valid';
      }
    }

    if (successCount === logsToSync.length) {
      setBulkResult({
        success: true,
        message: `Sukses penuh! Seluruh (${successCount}) data pembayaran lokal berhasil ditambahkan dan disinkronkan ke Spreadsheet Anda.`
      });
    } else {
      setBulkResult({
        success: false,
        message: `Sinkronisasi selesai dengan gangguan: ${successCount} dari ${logsToSync.length} berhasil diunggah. Kesalahan terakhir: ${failError}`
      });
    }
    ...
    <div className="space-y-6 text-left">
      ...
      {activeSubTab === 'koneksi' && (
        <div className="space-y-6 animate-fade-in font-sans">
          ...
          {/* Card untuk Sinkronisasi Manual Seluruh Data Lokal */}
          <div className="bg-white border border-slate-200 rounded p-5 shadow-xs space-y-4 transition-all hover:shadow-sm">
            <div className="border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-1.5">
                <CloudUpload className="w-4.5 h-4.5 text-blue-600 animate-pulse" />
                Sinkronisasi Massal Seluruh Riwayat Pembayaran ({getLocalLogsCount()} Riwayat)
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Gunakan fungsi ini jika Anda sebelumnya membuat pembayaran dalam mode offline (tanpa URL), atau ingin mengunggah paksa semua log audit keuangan lokal di browser ini ke file Google Sheets Anda.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="text-xs font-semibold text-slate-650">
                  Total data terdeteksi: <strong className="text-slate-900 font-bold bg-slate-50 border px-1.5 py-0.5 rounded font-mono">{getLocalLogsCount()} transaksi</strong>
                </div>

                <button
                  onClick={handleBulkUpload}
                  disabled={bulkLoading || getLocalLogsCount() === 0}
                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2.5 rounded transition duration-150 flex items-center gap-1.5 uppercase tracking-wider border border-blue-700/40 cursor-pointer disabled:opacity-50"
                >
                  {bulkLoading ? (
                    <>
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                      Sedang Sinkronisasi ({bulkProgress.current}/{bulkProgress.total})
                    </>
                  ) : (
                    <>
                      <CloudUpload className="w-3.5 h-3.5" />
                      Mulai Kirim Seluruh Data
                    </>
                  )}
                </button>
              </div>
              ...
              {bulkResult && (
                <div className={`p-4 rounded border animate-fade-in ${
                  ...
                  <div className="flex gap-2.5 items-start">
                    ...
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider m-0 font-sans">
                        {bulkResult.success ? 'Proses Sinkronisasi Sukses' : 'Gagal Menyinkronkan Sebagian Data'}
                      </h4>
                      <p className="text-xs font-medium leading-relaxed font-sans mt-0.5">{bulkResult.message}</p>
                    </div>
                  </div>
                </div>
              )}
